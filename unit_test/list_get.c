...
{
	int first_item_index = rand() % size;
	int second_item_index = rand() % size;

	t_chain_lst *first_item = ft_chain_get(head, first_item_index);
	t_chain_lst *second_item = ft_chain_get(head, second_item_index);
	...
	v1 = *(int *)first_item->content;
	v2 = *(int *)second_item->content;
	if (v1 != first_item_index)
	{
		printf("F");
		return ;
	}
	if (v2 != second_item_index)
	{
		printf("F");
		return ;
	}
